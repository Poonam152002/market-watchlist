...
function calculateChange(
  ...
  if (Math.abs(change) >= 3) {
    score += 30;
    reasons.push(
      `Price moved ${Math.abs(change).toFixed(2)}% since your last check`
    );
  }
  ...
  if (volume >= averageVolume * 2) {
    score += 25;
    reasons.push(
      `Volume is ${(volume / averageVolume).toFixed(1)}x normal`
    );
  }
  ...
  if (newsCount > 0) {
    reasons.push(
      `${newsCount} new update${newsCount > 1 ? "s" : ""} since your last check`
    );
  }
...
app.get("/api/health", (_req, res) => {
  res.json({
    success: true,
    message: "Smart Market backend is running"
  });
});
...
app.post("/api/watchlist", (req, res) => {
  ...
  if (!market.some((s) => s.symbol === symbol)) {
    return res.status(404).json({
      success: false,
      error: "Stock is not available in the demo market"
    });
  }
...
app.listen(Number(PORT), "0.0.0.0", () => {
  console.log(
    `Smart Market backend running on port ${PORT}`
  );
});